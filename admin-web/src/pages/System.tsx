...
export default function System(){
  ...
  const updateHint = !upd ? "加载中…"
    ...
    : !upd.installed ? "当前为本地开发模式运行（非安装包形态），无法在线更新。安装正式版后可用。"
        ...
        <Col xs={24} xl={12}>
          <Card title={<span><SyncOutlined style={{color:"#1677ff", marginRight:8}}/>应用更新</span>} style={{borderRadius:16, boxShadow:"0 2px 12px rgba(0,0,0,0.04)", height:"100%"}}>
            <Space direction="vertical" size={12}>
              <Typography.Text>当前版本 <Typography.Text code>{upd?.currentVersion||info.version||"?"}</Typography.Text>。从更新源检查并安装新版本（桌面版走 Releases，Node 版走 npm registry）。</Typography.Text>
              <Space>
