...
- **桌面版首装设置页不再空白**：Rust 桌面版的配置读取链路此前没有 npm 版的「默认值兜底」——首装时 `config.json` 不存在，`audit`/`backups`/`security` 全为 `None`，设置与安全策略接口直接序列化成全 `null` 返回，表单一片空白；且全局命令黑名单为 `None` 时高危命令拦截完全不生效。现在 `config::load()` 出口对缺省字段统一回填默认值（审计开启、保留 30 天、备份保留 30 天/最多 20 份/默认关闭定时、黑名单 7 条高危规则），数值与 npm 版 `services/defaults.ts` 锁定一致，`/admin/api/defaults` 亦改为引用同一常量消除双份硬编码；用户显式配置（含显式清空黑名单、显式关闭审计）不受影响
- **行为变化（与 npm 版对齐的预期语义）**：首装/旧配置下审计日志保留从「永久」变为默认 30 天；桌面版首装即启用 7 条高危命令黑名单（`rm`/`shutdown`/`reboot`/`halt`/`poweroff`/`mkfs`/`dd`）
